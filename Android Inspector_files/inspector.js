    ...
    $.ajax({
        ...
        .done(function (data) {
                  ...
                  window.setTimeout(function () {
                      me.log.info("AK :: Calling resize() now... from done");
                      resize();
                  }, 1000);
              })
        .fail(function () {
                  ...
                  window.setTimeout(function () {
                      me.log.info("AK :: Calling resize() now... from fail");
                      resize();
                  }, 1000);
              });
}
...
Inspector.prototype.transformAutXmlToAjax = function (xmlDoc) {
    ...
    var transformNode = function(node) {
        ...
        var toCoordinate = function (strPresentation, posi) {
            if (strPresentation) {
                var bounds = strPresentation.replace("][", ",").replace("[", "").replace("]", "").split(",");
                var coordinates = {
                    "x": Math.round(parseFloat(bounds[0])),
                    "y": Math.round(parseFloat(bounds[1])),
                    "w": Math.round(parseFloat(bounds[2])) - Math.round(parseFloat(bounds[0])),
                    "h": Math.round(parseFloat(bounds[3])) - Math.round(parseFloat(bounds[1]))
                };
                return coordinates[posi];
            } else {
                return 0;
            }
        }
        var toPrettyNodeName = function(node) {
            if (!!node.getAttribute("text")) {
                return "[" + node.nodeName + " :: " + node.getAttribute("text") + "]";
            } else {
        ...
        var nodeAsJson = {"data": toPrettyNodeName(node),
            ...
            "metadata": {
                ...
                "name": node.getAttribute("resource-id"),
                ...
                "error": "",
                "rect": {
                    "x": toCoordinate(node.getAttribute("bounds"), "x"),
                    "y": toCoordinate(node.getAttribute("bounds"), "y"),
                    "h": toCoordinate(node.getAttribute("bounds"), "h"),
                    "w": toCoordinate(node.getAttribute("bounds"), "w")
                },
            },
            "children": new Array()
        };
